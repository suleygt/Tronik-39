...
function handleKeyPress(event) {
  ...
  const synthKey = synthKeys.find((keyObj) => keyObj.computerKey === key);

  if (synthKey) {
    setSynthKeys((prevKeys) =>
      prevKeys.map((keyObj) =>
        keyObj === synthKey ? { ...keyObj, active: true, keyPressed: true } : keyObj
      )
    );
  }
}
...
function handleKeyRelease(event) {
  ...
  if (synthKey) {
    setSynthKeys((prevKeys) =>
      prevKeys.map((keyObj) =>
        keyObj === synthKey ? { ...keyObj, keyPressed: false } : keyObj
      )
    );
  }
}
...
function handleKeyClick(event) {
  ...
  const synthKey = synthKeys.find((keyObj) => keyObj.keyName === clickedKey);

  if (synthKey) {
    setSynthKeys((prevKeys) =>
      prevKeys.map((keyObj) =>
        keyObj === synthKey ? { ...keyObj, active: true } : keyObj
      )
    );
  }
}
...
return (
  <div className='wrapper'>
    <div className='main-container' tabIndex={0} onKeyDown={(e) => handleKeyPress(e)} onMouseDown={(e) => handleKeyClick(e)} onKeyUp={(e) => handleKeyRelease(e)}>
